...
from sjm_pkg.udpComms import Udp_rebroadcaster
...
class ICLlogger(QMainWindow):
    ...
    def on_new_icl_record(self, msg):
        ...
        if housing_is_ok and tip_is_ok:
            ...
            recast_msg = ("ICL "+ mysystimef3() + " " + self.tip_temp + " " + self.housing_temp + " C")
            self.udprb.send_datagram(recast_msg)
...
def mysystimef3():
    now = QDateTime.currentDateTime()
    systime = now.toString("yyyy/MM/dd hh:mm:ss.zzz")
    return systime
